function z = getWtMap(models,mpc,expt,options,varargin)
...
% u = u_admin; disp(u);
% u = u_admin([1,2,6,8,12,14,16],:); disp(u);
u = u_admin([15],:); disp(u);
...
models = models(:); expt = expt(:);
Nm = numel(models); rnge = mpc.alg.rnge_u;

% Add sparse grid toolboxes to path
sgipaths();

% Specify default interpolation options
sgiOptDef = spset( ...
    'GridType','Clenshaw-Curtis', ...% Sparse grid basis functions
    ...
    'NumberOfOutputs',Nm, ...       % Number of multi-output grids
...
sgiOpt = []; if isfield(options,'sgiOpt'), sgiOpt = options.sgiOpt; end
...
z = sgi(ParallelObj,rnge,zPast,sgiOpt);
%}
...
% Identify neighboring experiments and compute linear weights
[exptWt,exptIndU,exptInd] = getExptWeights(u,expt);
exptInd_active = find(exptWt ~= 0); exptWt = exptWt(exptInd_active);
expt_active = expt(exptIndU(exptInd_active));
EXPT_active = expt(cat(1,exptInd{exptInd_active}));
% Evaluate data interpolant to generate implicit data
[T_obs,Y_obs] = getInterpData(u,EXPT_active);
% Evaluate model to generate state dynamics
[T,Y] = getModTraj(u,models,mpc,expt_active,T_obs,varargin{:});
% Evaluate objective function to estimate error residuals
F = Objective_WLSE(Y,T,Y_obs,T_obs,exptWt);
...
function [exptWt,exptIndU,exptInd] = getExptWeights(u,expt)
%GETEXPTWEIGHTS Computes weights that emphasize experimental data collected
%   under experimental conditions similar to the computed input sequence.
%   
%   INPUTS:
%   u: [vector] input sequence.
%   expt: [structure] contains experimental input sequences for all
%       experiments for which data are available.
%   
%   OUTPUTS:
%   exptWt: [vector] weighting factors describing the emphasis placed on
%       each experiment in relation to all of the others.
%   exptIndU: [vector] indices denoting the subset of expt that are
%       representative of the different possible experiments.
%   exptInd: [cell] indices denoting the subsets of expt corresponding to
%       the different possible experiments.
%   
%   Written by: Jeffrey Perley (jperley@purdue.edu)
%   Last revision: 7/9/2012


% Determine indices of representatives of each unique experiment type
Nexpt = numel(expt);                % Number of experiments
names = arrayfun(@(x)x.name,expt,'uniformoutput',0);% Sources for all expts
[~,indU,indA] = unique(names,'first');% Names of unique expts types
exptIndU = sort(indU);              % First indices of unique expts types
exptInd = mat2cell((1:Nexpt)',diff([exptIndU;Nexpt+1]));% Partitioned indices
u0 = cell2mat(arrayfun(@(x)x.u_admin,expt,'uniformoutput',0));% Expt inputs
indM = find(ismember(u0,u,'rows')); % Match input to existing expts

% Compute weights for each experiment type
exptWt = zeros(1,numel(exptIndU));  % Initialize linear weighting vector
if ~isempty(indM)                   % IF input matches existing expt
    exptWt(ismember(exptIndU,indU(indA(indM)))) = 1;% Set expt weights to 1
else                                % ELSE input matches no existing expt
    u_active = u ~= 0;              % Find indices of active inputs
    if isequal(u_active,[1,0])      % IF first input active only
        exptWt = [1,0,0];           % First input gets maximum weight
    elseif isequal(u_active,[0,1])  % ELSEIF second input active only
        exptWt = [0,1,0];           % First input gets maximum weight
    else                            % ELSE both inputs active
        exptWt = [0.1,0.1,0.8];     % Weight interaction expts heavily
    %     exptWt = [0.5,0.5,0.0];     % Weight data on axes evenly
    end                             % IF first input active only
end                                 % IF input matches existing expt
exptWt = exptWt/sum(exptWt);        % Normalized weights summing to unity
...
allOneFig = 0;% Use subplots if true, use separate figures if false
