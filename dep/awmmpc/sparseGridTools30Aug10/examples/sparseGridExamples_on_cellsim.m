...
% Set up the sparse grid toolbox - change to reflect your file structure
spPath = '\\bmelab.ecn.purdue.edu\cellsim\Sparse_grids\spinterp_v5.1.1';

% Set up the sparse grid tools folder - change to reflect your file structure
saPath = '\\bmelab.ecn.purdue.edu\cellsim\Sparse_grids\Greg\sparseGridTools30Aug10';
...
dim = 4;  % dimension of uncertain parameter space
fun = @testFcnLag;  % call to objective function 
range = [-1.1 1.2; -1.3 .9; -.7 1.2; -1.1 1.1];  %parameter ranges
numOutput = 3;  % number of model outputs to be used
curOutput = 2;  % selector function to indicate which output comparison or special computation done
