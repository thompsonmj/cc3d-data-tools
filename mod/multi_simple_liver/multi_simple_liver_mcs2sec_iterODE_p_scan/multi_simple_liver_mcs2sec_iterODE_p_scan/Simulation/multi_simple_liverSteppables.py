...
class multi_simple_liverSteppable(SteppableBasePy):    
    ...
    def start(self):
        ...
        titleLine += "\tthep1.dict['APAP']\thep1.dict['APAPG']\thep1.dict['APAPS']"
        titleLine += "\thep1.dict['APAPG']\thep1.dict['APAPS']"
        titleLine += "\tblood1.dict['APAP']\tblood1.dict['APAPG']\tblood1.dict['APAPS']"
        titleLine += "\tPBPKspeciesDictAPAP['CLiver']"
        titleLine += "\tPBPKspeciesDictAPAPS['CLiver']"
        titleLine += "\tPBPKspeciesDictAPAPG['CLiver']"
        titleLine += "\tPBPKspeciesDictAPAP['CVen']"
        titleLine += "\tPBPKspeciesDictAPAPS['CVen']"
        titleLine += "\tPBPKspeciesDictAPAPG['CVen']"
        titleLine += "\tSUBCELLspeciesDict['APAP']"
        titleLine += "\tSUBCELLspeciesDict['APAPconj_Glu']"
        titleLine += "\tSUBCELLspeciesDict['APAPconj_Sul']\n"
        self.fileHandleLog.write(titleLine)
            
        # make a hepatocyte and a blood portion "cell"
        # relative areas (volumes) of 15:85
        ...
        #  10 to big, 0.1 fails, 0.01 fails, 0.001 fails, 0.5 runs, 1.0 runs
        self.mcs2second = p.cc3d_mcs2second # 1.0 simulated second per mcs (second/mcs)
        
        self.setMaxMCS(int(28800/self.mcs2second + 1))
...
# # #         self.setSBMLGlobalOptions(SBMLoptions)    
        ...
        initialConditions1['Fraction_unbound_plasma'] = p.pbpk_Fup
        initialConditions1['CLmetabolism'] = p.CLmetabolism  ### turn off the PBPK-SBML model's metabolism ODE (since the subcellular does it)
        initialConditions1['Kliver2plasma'] = p.pbpk_Kl2p  ###
        initialConditions1['VLiver'] = p.VLiver ### changed the volume of the liver to blood only
        ...
        initialConditions2['Fraction_unbound_plasma'] = p.pbpk_FupS
        initialConditions2['CLmetabolism'] = p.CLmetabolism  # turn off the PBPK-SBML model's metabolism ODE (since the subcellular does it)
        initialConditions2['VLiver'] = p.VLiver # changed the volume of the liver to blood only
        ...
        initialConditions3['Fraction_unbound_plasma'] = p.pbpk_FupG
        initialConditions3['CLmetabolism'] = p.CLmetabolism  # turn off the PBPK-SBML model's metabolism ODE (since the subcellular does it)
        initialConditions3['VLiver'] = p.VLiver # changed the volume of the liver to blood only
        self.addFreeFloatingSBML(_modelFile=modelFile1,_modelName='PBPK_APAPG',_stepSize=timeStep1,_initialConditions=initialConditions3)    
        ...
        SUBCELLspeciesDict=self.getSBMLState(_modelName='METAB_APAP',_cell=hep1) # returns dictionary of values for the single cell
        ...
        if self.doPlot:
            self.pW1=self.addNewPlotWindow(_title='Plasma Concentrations',_xAxisTitle='Time (hours)',_yAxisTitle='ug/ml', \
                _xScaleType='linear',_yScaleType='linear')
            self.pW1.addPlot('APAP',     _style='Lines',_color='red',  _size=2)
            self.pW1.addPlot('APAPG',    _style='Lines',_color='yellow', _size=2)
            self.pW1.addPlot('APAPS',    _style='Lines',_color='green',_size=2)
            self.pW1.addPlot('APAP_exp', _style='Dots', _color='red',  _size=4)
            self.pW1.addPlot('APAPG_exp',_style='Dots', _color='yellow', _size=4)
            self.pW1.addPlot('APAPS_exp',_style='Dots', _color='green',_size=4)
            # plot the t=0 points
            ...
            self.ADME=( (0.0, 0.0, 0.0,  0.0),  \
                        ...
                        (12.0,  1.20,0.72, 2.05)) # truncate data to the first 12 hours, 
                        #(24.0,0.38, 0.12,0.27) )
            ...
            self.pW2=self.addNewPlotWindow(_title='Total Recovered APAP Equivalent (in Tubules Only)',_xAxisTitle='Time (hours)',_yAxisTitle='Dose APAP Equiv', \
                _xScaleType='linear',_yScaleType='linear')
            self.pW2.addPlot('Total', _style='Lines',_color='Red',   _size=1)
            self.pW2.addPlot('Theory',_style='Lines',_color='Green', _size=2)

            self.pW3=self.addNewPlotWindow(_title='In Hep1',_xAxisTitle='Time (hours)',_yAxisTitle='Molarity', \
                _xScaleType='linear',_yScaleType='linear')
            self.pW3.addPlot('APAP', _style='Lines',_color='Red',   _size=2)
            self.pW3.addPlot('APAPG',_style='Lines',_color='yellow', _size=2)
            self.pW3.addPlot('APAPS',_style='Lines',_color='Green',  _size=2)
    ...
    def step(self,mcs):
        ...
        if mcs % 1 == 0:        
            ...
            # Units are MOLES, need to convert to milliMolar using the liver's volume
            PBPKspeciesDictAPAP =self.getSBMLState(_modelName='PBPK_APAP')
            PBPKspeciesDictAPAPS=self.getSBMLState(_modelName='PBPK_APAPS')
            PBPKspeciesDictAPAPG=self.getSBMLState(_modelName='PBPK_APAPG')
            ...
            # convert to millimole/liter
            blood1.dict['APAP']  = PBPKspeciesDictAPAP['CLiver'] *1000./PBPKspeciesDictAPAP['VLiver'] # APAP concentration (millimole/Liter) in liver compartment
            blood1.dict['APAPS'] = PBPKspeciesDictAPAPS['CLiver']*1000./PBPKspeciesDictAPAP['VLiver']
            blood1.dict['APAPG'] = PBPKspeciesDictAPAPG['CLiver']*1000./PBPKspeciesDictAPAP['VLiver']

            # update the Hep from the subcellular model
            hep1.dict['APAP'] =SUBCELLspeciesDict['APAP']  # SBML is mM concentration
            hep1.dict['APAPG']=SUBCELLspeciesDict['APAPconj_Glu']  # SBML is mM concentration
            hep1.dict['APAPS']=SUBCELLspeciesDict['APAPconj_Sul']  # SBML is mM concentration
            ...
            hepatoVolume = 8e-15 # Liter
            bloodVolume = p.bloodVolume  # 1.0e-15 # Liter
            hepOverBloodVolumeRatio = 8e-15/p.bloodVolume
            #
            ...
            for iii in range(1,p.cc3d_iterODE+1):  # extra steps for the CC3D ODE transfer calculations
                ...
                hep1.dict['APAP']   += blood2hepFlux *bloodVolume/hepatoVolume
                blood1.dict['APAP'] += hep2bloodFlux *hepatoVolume/bloodVolume
    ...
    #             p.cc3d_Vmax_AT_APAP = 0.005  # 0.01 mmol/L/s
    #             p.cc3d_Km_AT_APAP   = 0.01  # 0.01 mmol/L
                APAPactFlux = p.cc3d_Vmax_AT_APAP*blood1.dict['APAP']*p.pbpk_Fup/p.pbpk_Rb2p \
                    ...
                    * bloodHepSurfaceArea *self.mcs2second  /p.cc3d_iterODE  ###4
                hep1.dict['APAP']   += APAPactFlux *bloodVolume/hepatoVolume
                ...
                METAB_S_actFlux = p.cc3d_k_AT_APAPS*hep1.dict['APAPS'] * \
                    bloodHepSurfaceArea *self.mcs2second /p.cc3d_iterODE ###4
                blood1.dict['APAPG'] += METAB_G_actFlux *hepatoVolume/bloodVolume
                blood1.dict['APAPS'] += METAB_S_actFlux *hepatoVolume/bloodVolume
                hep1.dict['APAPG']   -= METAB_G_actFlux
                hep1.dict['APAPS']   -= METAB_S_actFlux

            ############################## END iterate CC3D ODEs #####################################
            
#             print "mcs,hep2bloodFlux,blood2hepFlux,APAPactFlux,METAB_G_actFlux,METAB_S_actFlux:", \
#                 mcs,hep2bloodFlux,blood2hepFlux,APAPactFlux,METAB_G_actFlux,METAB_S_actFlux
            ...
            self.setSBMLValue(_modelName='PBPK_APAPG',_valueName='CLiver',_value=blood1.dict['APAPG']/1000.*PBPKspeciesDictAPAP['VLiver'])
            self.setSBMLValue(_modelName='PBPK_APAPS',_valueName='CLiver',_value=blood1.dict['APAPS']/1000.*PBPKspeciesDictAPAP['VLiver'])
        ...
        if self.doPlot & (mcs % 300 == 0) :
            ...
            self.pW1.addDataPoint("APAP", aTime,self.APAPug_ml ) # Need the MW's, APAP
            self.pW1.addDataPoint("APAPG",aTime,self.APAPGug_ml) # APAP-Glucuronide
            self.pW1.addDataPoint("APAPS",aTime,self.APAPSug_ml) # APAP-sulfate

            totAPAPequivTubMoles = PBPKspeciesDictAPAP['CTubules'] + PBPKspeciesDictAPAPG['CTubules'] \
                + PBPKspeciesDictAPAPS['CTubules']
