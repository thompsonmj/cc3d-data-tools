...
class multi_simple_liverSteppable(SteppableBasePy):    
    ...
    def start(self):
        # delete any results.txt file (used in swarm, uses the same directory for multiple simulations)
        filePath = os.path.abspath(__file__)
        rFile = filePath+'results.txt'
        if os.path.exists(rFile): os.remove(rFile)          
...
###       self.setSBMLGlobalOptions(SBMLoptions)    
        ...
        modelFile1 = '/depot/dumulis/cc3d/apps/CC3D_v377/_models/multi_simple_liver/Simulation/BIOMD0000000619.xml' # time unit=HOUR, substance=MOLE, volume=LITER, 
        ...
        modelFile2 = '/depot/dumulis/cc3d/apps/CC3D_v377/_models/multi_simple_liver/Simulation/BIOMD0000000624.xml' # time unit=SECONDS, substance=MILLIMOLE, volume=LITER, 
...
# vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
            hepOverBloodVolumeRatio = 1.  # 6.15, 1.7, 1, 1/1.7
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#                 print "blood1.dict['APAP'],hep1.dict['APAP']:\n",blood1.dict['APAP'],hep1.dict['APAP']
	...
	#####################################################################
        # write a file containing just the ChiSq value
        try:                
            aFile,aFilefullFileName=self.openFileInSimulationOutputDirectory("result.txt","w")
        except IOError:
            print "Could not open file for writing result.txt data. "                
            return
        aFile.write(str(self.accumChiSquaredTotal))
        aFile.close()